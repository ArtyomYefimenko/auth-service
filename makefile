...
make-migration:
	docker exec -it $(SERVICE_NAME) bash -c "alembic revision --autogenerate"

migrate:
	docker exec -it $(SERVICE_NAME) bash -c "alembic upgrade head"

downgrade:
	docker exec -it $(SERVICE_NAME) bash -c "alembic downgrade -1"

psql:
	$(COMPOSE_CMD) exec auth-service-postgres psql -U postgres
